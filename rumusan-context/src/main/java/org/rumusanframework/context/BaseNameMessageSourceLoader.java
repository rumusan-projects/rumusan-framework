...
public class BaseNameMessageSourceLoader {
  ...
  public static void addBasename(ApplicationContext applicationContext,
    ...
    List<BaseNameMessageSource> baseNameMessageSources = getBaseNameMessageSource(
        applicationContext);

    if (!baseNameMessageSources.isEmpty()) {
      System.out.println("Found " + baseNameMessageSources.size()
          + " of autowired basename message source.");// NOSONAR
    }
  ...
  private static List<BaseNameMessageSource> getBaseNameMessageSource(
      ApplicationContext applicationContext) {
    Map<String, BaseNameMessageSource> obj = applicationContext
        .getBeansOfType(BaseNameMessageSource.class);
